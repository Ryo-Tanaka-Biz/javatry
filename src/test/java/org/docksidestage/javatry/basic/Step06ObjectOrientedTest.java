...
public class Step06ObjectOrientedTest extends PlainTestCase {
    ...
    // done jflute OSクラスを使って再利用の思考のエクササイズをします↑ (2024/11/15)
    // [1on1でのふぉろー] まず、UserDirectory が同じ値だけど再利用すべきかどうか？概念的に考えましょう。
    // の話をした。
    // → 「再利用は、コードが同じだからするんじゃなく、意味が同じだからする」
    // [1on1でのふぉろー] FileSeparatorのほうの話もした。
    // 修正時に両方直すわけじゃないかもだけど、意味が同じであれば可読性のために定数化などしておいても良い。
    // 一方で、WindowsとOldWindowsのクラス構造の違和感についても議論した。
    // 概念化が大事なスキル。(概念性とでも言うのか？)
    ...
    public void test_objectOriented_zoo() {
        ...
        // done tanaryo [読み物課題] 理想論は最下位と表裏一体 by jflute (2024/12/06)
        ...
        //
        // [1on1でのふぉろー]「合成の誤謬」の話をした。何が最善って実は難しい。by jflute (2024/12/13)
        // 最善と思っていたものが視点を変えれば最善じゃないかも知れない。(その理想論は本当に理想か？)
        //
        // done tanaryo [読み物課題] リファクタリングは思考のツール by jflute (2024/12/06)
        // https://jflute.hatenadiary.jp/entry/20121202/1354442627
        //メモ：操作方法に気を取られて本当にやりたいこと（思考するとか）に集中できないのは勿体無い！（by tanaryo）
        // done tanaryo [読み物課題] 別に、プルリクレビューの前にレビューしてもらっていいんだからね by jflute (2024/12/06)
        // https://jflute.hatenadiary.jp/entry/20170630/reviewbefore
        // 自分が担当しているタスクの進捗が周囲から見てブラックボックス化していると、盛大な手戻りや連携ミスのリスクが発生して良くないなあと
        // 周囲のタスク状況を把握する役割を与えられてからつくづく感じます。なので自分がメインの作業者の場合、つぶやきレベルでもいいから自分が考えていることをオープンな場所にさらすようにしています笑
        // 紙とペン by tanaryo
        // フリーハンドの話 by jflute
        // done tanaryo [読み物課題] ホワイトボードを買ってこよう by jflute (2024/12/06)
